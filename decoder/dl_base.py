...
def printacc(score_mat, K = 5, X_Y = None, disp = True, inv_prop_ = None):
    ...
    recall = xc_metrics.recall(score_mat, X_Y, K)*100
    recall_at_gt = xc_metrics.recall_at_gt(score_mat, X_Y, pad_val=512)*100
    recall_dict = {f"R@{i+1}": value for i, value in enumerate(recall)}
    recall_dict.update({f"R@GT":recall_at_gt})
...
class FP32Linear(nn.Module):
    ...
    def forward(self, input):
        ...
        with torch.cuda.amp.autocast(enabled=False):
            ...
            # out = F.linear(['xfts']['input_ids'].float(), self.transformer_weight, self.transformer_bias) 
        return out
...
class GenericModel(nn.Sequential):
    ...
    def fit(self,
        ...
        if self.checkpoint_resume != '':
          if os.path.isfile(f'{self.checkpoint_resume}/{self.args.data_dir}_{self.args.seed}_{self.args.num_layers}/{self.name}_{self.rank}_checkpoint.pt'):
        ...
        for epoch in trange(start_epoch, epochs, desc="Epoch", initial=start_epoch, total=epochs):
            ...
            eval_results = {}
            eval_results['train/total_loss'] = total_loss
            eval_results['train/mean_loss'] = mean_loss
    ...
    def checkpoint(self, loss):
        ...
        if os.path.exists(f'{out_dir}/{self.args.data_dir}_{self.args.seed}_{self.args.num_layers}/{self.name}_{self.rank}_checkpoint.pt'):
            os.rename(f'{out_dir}/{self.args.data_dir}_{self.args.seed}_{self.args.num_layers}/{self.name}_{self.rank}_checkpoint.pt',f'{out_dir}/{self.args.data_dir}_{self.args.seed}_{self.args.num_layers}/{self.name}_{self.rank}_checkpoint_prev.pt')
        os.makedirs(f'{out_dir}/{self.args.data_dir}_{self.args.seed}_{self.args.num_layers}', exist_ok=True) 
        torch.save(save_dict, f'{out_dir}/{self.args.data_dir}_{self.args.seed}_{self.args.num_layers}/{self.name}_{self.rank}_checkpoint.pt')

    def resume(self):
        out_dir = self.checkpoint_resume
        load_dict = torch.load(f'{out_dir}/{self.args.data_dir}_{self.args.seed}_{self.args.num_layers}/{self.name}_{self.rank}_checkpoint.pt')
    ...
    def loadFromCheckpoint(self, out_dir: str = None):
        self.load_state_dict(torch.load(f'{out_dir}', map_location=self._target_device)['model_dict'])
...
class SimpleEncoder(nn.Module):
    def __init__(self, input_size):
        ...
        # Define a fully connected layer with the specified input size and an output size of 768
        self.fc = nn.Linear(in_features=input_size, out_features=768)
...
class BCELossMultiNode(nn.Module):
    ...
    def forward(self, batch_data):
        # import pdb; pdb.set_trace()
        embed = self.model.embed(batch_data['xfts']['input_ids']) #FIXED
        # embed = batch_data['xfts']['input_ids']
        return embed
        ...
        with torch.no_grad():
            for step, batch_data in enumerate(tqdm(dataloader, desc="Evaluating",disable=(model.rank != 0))):
                ...
                with torch.cuda.amp.autocast(enabled=model.fp16encoder):
                  embed_out = model.embed(batch_data['xfts']['input_ids'])  # FIXED
...
class PrecEvaluator():
    def __init__(self, model: GenericModel, args,  dataloader, predictor, filter_mat = None, K=5, metric='P', inv_prop=-1):
        self.K = K
        self.metric = metric
    ...
    def __call__(self, loss_model, epoch: int = -1, loss: float = -1.0, out_dir: str = None, name: str = ''):
        ...
        if self.model.rank == 0:
          ...
          res, recall= printacc(score_mat, X_Y=self.dataloader.dataset.labels, K=max(5, self.K), disp=False, inv_prop_=self.inv_prop) 
        ...
        if self.model.rank == 0 and out_dir is not None:
            ...
            import pdb; pdb.set_trace()
            if self.args.infer:
                predicted_label_file = f'{out_dir}/{[name+"_", ""][name == ""]}_predicion.csv'
                predicted_tokens = xc_metrics._get_topk(score_mat, self.dataloader.dataset.labels.shape[1], max(5, self.K))
                ground_truth = xc_metrics._get_topk(self.dataloader.dataset.labels, self.dataloader.dataset.labels.shape[1], max(5, self.K))
                
                threshold = -2
                modifiedPredictedTokens = predicted_tokens
                for i in range(modifiedPredictedTokens.shape[0]):  
                    for j in range(modifiedPredictedTokens.shape[1]):  
                        index = modifiedPredictedTokens[i, j]
                        if index < self.dataloader.dataset.labels.shape[1]: 
                            if score_mat[i, index] < threshold: 
                                modifiedPredictedTokens[i, j] = self.dataloader.dataset.labels.shape[1]  

                df1 = pd.DataFrame(predicted_tokens, columns=[f'pred_t{i+1}' for i in range(5)])
                df2 = pd.DataFrame(ground_truth, columns=[f'ground_t{i+1}' for i in range(5)])

                df1['Empty_Column'] = ''
                combined_df = pd.concat([df1, df2], axis=1)
                combined_df = pd.concat([df1, df2], axis=1)
                combined_df.to_csv(predicted_label_file, index=False)
                print(f'dumping prediction in {predicted_label_file}')
        ...
        if self.args.use_wandb:
            self.args.wandb.log(res.to_dict())
            self.args.wandb.log(res.to_dict()['1'])
            self.args.wandb.log(res.to_dict()['2'])
            self.args.wandb.log(res.to_dict()['3'])
            self.args.wandb.log(res.to_dict()['4'])
            self.args.wandb.log(res.to_dict()['5'])
            self.args.wandb.log(recall)
            self.args.wandb.log_model(path=out_dir, name=f'{self.model.name}_{self.model.rank}_state_dict.pt')
        return score
...
class PreTokBertDataset(torch.utils.data.Dataset):
    ...
    def get_fts_old(self, indices, source='point'):
        if self.start: # doing the memmap in init duplicates it across processes when num_workers > 0; OOM for large datasets; somehow doing it here avoids this issue!
           ...
           self.X_am = np.memmap(f"{self.tokenization_folder}/{self.doc_type}_doc_attention_mask.dat",
                               mode='r', shape=(self.num_points, self.max_len), dtype=np.int64)
        print(self.X_ii.shape)
        print(self.X_am.shape)
        if source == 'point':
    ...
    def get_fts(self, indices, source='point'):
        if self.start: # doing the memmap in init duplicates it across processes when num_workers > 0; OOM for large datasets; somehow doing it here avoids this issue!
            ...
            if "Float" in self.tokenization_folder:
                self.X_ii = np.memmap(f"{self.tokenization_folder}/{self.doc_type}_embeddings.dat",
                                mode='r', shape=(self.num_points, self.max_len), dtype=np.float32)
            else:
                self.X_ii = np.memmap(f"{self.tokenization_folder}/{self.doc_type}_embeddings.dat",
                                mode='r', shape=(self.num_points, self.max_len), dtype=np.int32) 
            self.X_am = np.memmap(f"{self.tokenization_folder}/{self.doc_type}_mask_embeddings.dat",
                                mode='r', shape=(self.num_points, self.max_len), dtype=np.int8)
            # print(self.X_ii)
        if source == 'point':
            return bert_fts_batch_to_tensor(self.X_ii[indices], self.X_am[indices])
        if source == 'label':
...
class BertEmbedder(nn.Module):
    def __init__(self, embed_size=108, hidden_size=768, num_layers=1, num_heads=1,  dropout_prob=0.1):
        ...
        self.transformer_layers = nn.ModuleList([
            nn.TransformerEncoderLayer(embed_size, num_heads, dim_feedforward=hidden_size, dropout=dropout_prob)
            for _ in range(num_layers)
        ])

        self.output_layer = nn.Linear(embed_size, hidden_size)

    def forward(self, input_embeddings):
        # import pdb; pdb.set_trace()
        # print(input_embeddings)
        embeddings = self.layer_norm(input_embeddings.float())

        for transformer_layer in self.transformer_layers:
            embeddings = transformer_layer(embeddings)

        output_embeddings = self.output_layer(embeddings)
