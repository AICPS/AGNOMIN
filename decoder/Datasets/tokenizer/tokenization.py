...
#             token_list.pop(0)
    ...
    def  processList( functionList) :
        ...
        for name in tqdm(functionList, desc = "Tokenization Progress...") :
            counter += 1
            print(counter, ", ", name)
            split_list = Function.splitFunctionName(name)
            f = Function(name)
            f.addTokenList(split_list)
            for str1 in tqdm(split_list, desc = "split_list processing...") :            
...
if __name__=="__main__":
    #print('hi')
    cProfile.run('Function.main(sys.argv[1])', 'trunc_profiling')