...
'''This utils.py is from gemini's toolbox'''
...
class graph(object):
    def __init__(self, node_num = 0, label = None, name = None): # parameters are: amount of nodes in graph, class label of graph, name of function
...
def read_graph_allstar(function_graphs, func2label):
    # function_graphs[function name] = (features, edges, basic block names, label)
    graphs = []
    classes = []
    
    packages = defaultdict(list)
    
    for f in range(len(func2label.values())):
        classes.append([])

    for name, (features, edges, _, label) in tqdm(function_graphs.items()):
        #no_arch_name = '.'.join(name.split(".")[1:])
        #label = func2label[no_arch_name]
        #func_name = '.'.join(name.split(".")[2:])
        classes[label].append(len(graphs))
        packages[name[1]].append(len(graphs))
        num_nodes = features.shape[0]
        cur_graph = graph(num_nodes, label, name)

        for u in range(num_nodes):
            cur_graph.features[u] = np.array(features[u])
        
        for u, v in zip(edges[0], edges[1]):
            cur_graph.add_edge(u.item(), v.item())

        graphs.append(cur_graph)
    
    return graphs, classes, packages


def partition_data_packages(Gs, classes, partitions, packages, perm):
    C = len(packages)
    st = 0.0
    ret = []
    all_pkg_list = packages.keys()
    all_pkg_list.sort()
    for part in partitions:
        ...
        ed = st + part * C
        pkg_list = []
        for pkg_idx in perm[int(st):int(ed)]:
            pkg_list.append(all_pkg_list[pkg_idx])
        print(pkg_list)
        for pkg_name in pkg_list:
            list_pkg_g_idx = packages[pkg_name]
            for g_idx in list_pkg_g_idx:
                cur_g.append(Gs[g_idx])    
        ...
        for g_idx, g in enumerate(cur_g):
            g.label = label2idx[g.label]
            cur_c[g.label].append(g_idx)
...
# they split datasets by the classes
def partition_data(Gs, classes, partitions, perm):
    ...
    for part in partitions:
        ...
        for cls in range(int(st), int(ed)):
            prev_class = classes[perm[cls]] # prev_class is a list of the indices of functions in Gs that have that class
            cur_c.append([])
            for i in range(len(prev_class)):
                cur_g.append(Gs[prev_class[i]])
                cur_g[-1].label = len(cur_c)-1 #NOTE: they reassign a new label for graphs which affects Gs
                cur_c[-1].append(len(cur_g)-1) # because of this, classes is no longer index-based but merely keeps track of functions with same class
