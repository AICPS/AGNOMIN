...
class MultiArchSampler(BatchSampler):
    ...
    def __iter__(self):
        ...
        for i386_data_idx in shuffled_i386_list: 
            ...
            same_bag_sel[i386_data_idx] = 0.0 # avoid self selection.
        ...
        for amd64_data_idx in shuffled_amd64_list: 
            ...
            same_bag_sel = torch.tensor([1.0]*self.tot_data)
            same_bag_sel[amd64_data_idx] = 0.0 # avoid self selection.
            same_bag_idx = same_bag_sel.multinomial(num_samples=1).item()
            data.append(same_bag_idx)
            yield data
        ...
        for arm_data_idx in shuffled_arm_list: 
            ...
            same_bag_sel = torch.tensor([1.0]*self.tot_data)
            same_bag_sel[arm_data_idx] = 0.0 # avoid self selection.
            same_bag_idx = same_bag_sel.multinomial(num_samples=1).item()
            data.append(same_bag_idx)
            yield data
        ...
        for mips_data_idx in shuffled_mips_list: 
            ...
            same_bag_sel = torch.tensor([1.0]*self.tot_data)
            same_bag_sel[mips_data_idx] = 0.0 # avoid self selection.
            same_bag_idx = same_bag_sel.multinomial(num_samples=1).item()
            data.append(same_bag_idx)
            yield data
...
#         self.tolerance_count = tolerance
#         self.reset()

#     def reset(self):
#         self.curr_tolerance_count = 0
#         self.prev_loss = math.inf
      
#     def should_stop(self, curr_loss):
#         if self.tolerance_count == 0:
#             return False
#         elif self.prev_loss < curr_loss:
#             self.curr_tolerance_count = self.curr_tolerance_count + 1
#         else:
#             self.reset() # if loss is decreasing, reset current tolerance count so training is not stopped early
        
#         print("Early Stopper Stat(prev_loss, curr_loss, curr_count, max_count): %f,%f,%d,%d." % (self.prev_loss, curr_loss, self.curr_tolerance_count, self.tolerance_count))
#         self.prev_loss = curr_loss

#         return self.curr_tolerance_count >= self.tolerance_count
...
class HSNTrainer:
    ...
    def __init__(self, cfg, model, thunk_idx=0):
        ...
        self.model = model
        self.func_pred_loss = torch.nn.BCEWithLogitsLoss(reduction='sum')
        self.bce_loss = torch.nn.BCELoss(reduction='sum')
        self.BLEUScore_metric = BLEUScore(n_gram=1)
        if cfg.use_wandb: 
            self.wandb = wandb.init(project=self.cfg.wandb_project, dir="../../wandb", entity="aicpslab", config={}, name="acg_%s_%s_%d_%d"% (str(cfg.pickle_path)[:-4], cfg.included_archs, cfg.learning_rate, cfg.seed))
    ...
    def set_cluster_information(self, cluster_idx_test, cluster_idx_train, cluster_centers_test, cluster_centers_train):
        ...
        for cluster in range(1, self.cfg.num_clusters+1):
            cluster_filename = self.cfg.metrics_path / f"{'cluster_minitest_metrics_%d.csv'%cluster}"
    ...
    def train_model(self, train_data_loader, valid_data_loader, test_data_loader, idx2kw=None):
        ...
        for idx in tqdm_bar:
            ...
            for batch in train_data_loader:
                ...
                #acfg_embeds_kws_pred = self.model.func_predict(acfg_embeds)
                #acfg_kwds_list = (list(itertools.chain.from_iterable(batch.function_key_words)))
                #acfg_kwds_label_ = torch.tensor([[0.0]*self.model.num_func_keywords]).repeat(acfg_embeds_kws_pred.size(0),1).to(self.cfg.device)0
            ...
            if idx % self.cfg.test_step == 0:
                ...
                if self.do_cluster:
                    ...
                    # eval_results['train/train_avg_bleuScore'] = train_avg_bleuScore
                    for k in range(1,self.cfg.topk+1):
                        eval_results['test/p@%d'%k] = test_hits[k-1]
                    eval_results['test/loss'] = test_loss
                    self.wandb_or_print_results(eval_results)
                    
                    # with open(self.cfg.metrics_path / ('trainset_%d.log' % idx), "w") as log_file:
                    #     pprint.pprint(train_results, log_file, width=300)

                    with open(self.cfg.metrics_path / ('testset_%d.log' % idx), "w") as log_file:
                        pprint.pprint(test_results, log_file, width=300)

                   
                    embed_filename = self.cfg.metrics_path / ('func_embeds_epoch_%d.csv'%idx)
    ...
    def get_embeddings(self, data_loader, no_loss=False):
        ...
        with torch.no_grad():
            ...
            for batch in data_loader:
                ...
                function_key_words = (list(itertools.chain.from_iterable(batch.function_key_words)))                
                temp_names = [] 
                ...
                labels+=acfg_labels
                key_words += function_key_words               
                
        #import pdb; pdb.set_trace()
        return torch.cat(graph_embeddings, axis=0), torch.tensor(labels).to(self.cfg.device), names, final_loss, key_words

    def inference(self, data_loader, idx2kw=None, eval_set=None, no_loss=False):
        func_keywords = []
        func_embeds, labels, names, loss, _ = self.get_embeddings(data_loader, no_loss)
        # func_keywords.append(true_func_keywords)
        if eval_set != None:
            eval_func_embeds, eval_labels, eval_names, _, _ = self.get_embeddings(eval_set, no_loss)
    ...
    def do_func_matching(self, data_loader, eval_set=None, no_loss=False):
        func_embeds_stripped, stripped_labels, stripped_names, loss, _ = self.get_embeddings(data_loader, no_loss)

        func_embeds_unstripped, unstripped_labels, unstripped_names, _, _ = self.get_embeddings(eval_set, no_loss)
    ...
    def cluster_inference(self, data_loader, cluster_idx):
        function_embeddings, labels, names, loss, _ = self.get_embeddings(data_loader)
    ...
    def export_embeddings_labels_names_csv(self, filename, data_loader, filepath='.'):
        start = perf_counter()
        results = self.get_embeddings(data_loader)
        end = perf_counter()
        print(f"Generated embedding(s) in " + strftime("%H:%M:%S", gmtime(end - start)))
        print(f"which equals {end - start:0.4f} seconds")

        embeddings, names, labels = results[0], results[2], results[4]
        pd.DataFrame(data=embeddings.cpu().numpy()).to_csv(f'{filepath}/embeddings_{filename}.csv', header=False, index=False)
        pd.DataFrame(data=np.asarray(labels)).to_csv(f'{filepath}/labels_{filename}.csv', header=False, index=False)
        pd.DataFrame(data=np.asarray(names)).to_csv(f'{filepath}/names_{filename}.csv', header=False, index=False)

class EmbeddingDataset(Dataset):
        def __init__(self, embeddings, kw_labels):
            self.embeddings = torch.tensor(embeddings.astype(np.float64))
            self.kw_labels = torch.tensor(kw_labels)

        def __len__(self):
            return self.embeddings.shape[0]

        def __getitem__(self, idx):
            return tuple([self.embeddings[idx], self.kw_labels[idx]])
